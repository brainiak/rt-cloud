...
- **Project Specific Classification Script**
  ...
  - Waits for dicom files to be retrieved by the scannerDataService, creates a data model, returns classification results via the subjectInterface or scannerDataService for feedback purposes.
- **Subject Feedback (SubjectService)**
  ...
  - Uses the classification results in combination with a feedback toolkit and script such as PsychoPy, JsPsych, or PsychToolbox.
...
3. Copy the ssl certificate created in Step 2.4 above to this computer's rt-cloud/certs directory
    ...
    - Copy it into the rt-cloud/certs directory on the scannerDataService computer
...
3. Open another terminal to start the subjectService (where feedback is received)
...
3. ScannerDataService or subjectService can't connect to projectInterface.
    - Try specifying the --test option to all components (projectInterface, scannerDataService, subjectService). This will disable ssl and allow login with a test user, username: test, password: test. The web page will now be http://localhost:8888 (*not https://*)
    - Make sure the projectInterface firewall has port 8888 open. Try using an ssh tunnel if in doubt, 'ssh -N -L 8888:localhost:8888 [remote-computer]'
    - Try running the scannerDataService on the same computer as the projectInterface to test the connection.
    - Try using a different port, specify the --port option when starting the projectInterface and specify the appropriate port using -s [remote-computer]:[port] when starting the scannerDataService.
    - Make sure the ssl certificate and private key (rtcloud.crt and rtcloud_private.key) that were created on the projectInterface computer have been copied to the rtcloud/certs directory on the scannerDataService computer.
    - Make sure you have created a username and password using the 'scripts/add-user.sh' script.
4. ProjectInterface cannot find your experiment script. Make sure your script's name matches the project directory. Or specify the --mainScript [script-name] option when starting the projectInterface. In addition the --initScript and --finalizeScript options can be used to specify the session initialization and finalization scripts.
5. ProjectInterface or web page indicate 'RemoteServie: DataService not connected'. This means you started the projectInterface with the --dataRemote option specified but that a scannerDataService has not established a connection to the projectInterface, so it cannot make remote requests for data.
